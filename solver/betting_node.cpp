...
int TerminalNode2p::evaluate( const hand_t &hand, const int position,
                               const int utility_type, const double alpha,
                               const double beta, const double lambda ) const
{
  int chips = ( showdown ? hand.eval.showdown_value_2p[ position ]
                : fold_value[ position ] ) * money;

  double util = (double) chips;

  /* Apply utility distortion for player 1 only (the modified-utility player) */
  if( position == 1 && utility_type != 0 ) {
    if( utility_type == 1 ) {
      util += alpha * hand.p1_raises;
    } else if( utility_type == 2 ) {
      util -= beta * hand.p1_folds;
    } else if( utility_type == 3 ) {
      if( util < 0.0 ) {
        return (int)round( util * lambda * 100.0 );
      }
      return (int)round( util * 100.0 );
    }
    return (int)round( util );
  }
  return chips;
}
...
int TerminalNode3p::evaluate( const hand_t &hand, const int position,
                               const int utility_type, const double alpha,
                               const double beta, const double lambda ) const
{
