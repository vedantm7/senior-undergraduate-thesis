				   ...
				   rng_state_t &rng )
{
  ...
  if( player != position ) {
    ...
    if( player == 1 ) {
      if( choice == 0 ) { hand.p1_folds++; }
      else if( choice == 2 ) { hand.p1_raises++; }
    }
    ...
    int saved_raises = hand.p1_raises;
    int saved_folds  = hand.p1_folds;
    for( int c = 0; c < num_choices; ++c ) {
      hand.p1_raises = saved_raises;
      hand.p1_folds  = saved_folds;
      if( player == 1 ) {
        if( c == 0 ) { hand.p1_folds++; }
        else if( c == 2 ) { hand.p1_raises++; }
      }
