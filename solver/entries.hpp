...
int Entries_der<T>::load( FILE *file )
{
  ...
  if( type != this_type ) {
    /* Allow upcast from uint32 to uint64 (e.g. loading old Nash baseline) */
    if( type == TYPE_UINT32_T && this_type == TYPE_UINT64_T ) {
      uint32_t *tmp = new uint32_t[ total_num_entries ];
      num_read = fread( tmp, sizeof( uint32_t ), total_num_entries, file );
      if( num_read != total_num_entries ) {
        fprintf( stderr, "error while loading (upcast u32->u64); "
                 "only read %jd of %jd entries\n",
                 ( intmax_t ) num_read, ( intmax_t ) total_num_entries );
        delete[] tmp;
        return 1;
      }
      for( size_t i = 0; i < total_num_entries; ++i ) {
        entries[ i ] = ( T ) tmp[ i ];
      }
      delete[] tmp;
      fprintf( stderr, "note: upcasted avg-strategy round from uint32 to uint64\n" );
      return 0;
    }
