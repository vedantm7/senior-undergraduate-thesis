  ...
  for( ; index < argc; ++index ) {
	...
	if( !strcmp( abs_str, card_abs_type_to_str[ i ] ) ) {
      ...
      if( i >= NUM_CARD_ABS_TYPES ) {
        if( strncmp( abs_str, "FILE:", 5 ) == 0 ) {
          card_abs_type = CARD_ABS_FILE;
          strncpy( abs_file, abs_str + 5, PATH_LENGTH );
          abs_file[ PATH_LENGTH - 1 ] = '\0';
        } else {
...
void Parameters::print_params( FILE *file ) const
{
  ...
  if( card_abs_type == CARD_ABS_FILE ) {
    fprintf( file, "CARD_ABSTRACTION FILE:%s\n", abs_file );
  } else {
  ...
  while( fgets( line, PATH_LENGTH, file ) ) {
	...
	if( !strcmp( card_abs_str, card_abs_type_to_str[ i ] ) ) {
      ...
      if( card_abs_type == NUM_CARD_ABS_TYPES ) {
        ...
        if( strncmp( card_abs_str, "FILE:", 5 ) == 0 ) {
          card_abs_type = CARD_ABS_FILE;
          strncpy( abs_file, card_abs_str + 5, PATH_LENGTH );
          abs_file[ PATH_LENGTH - 1 ] = '\0';
        } else {
    ...
    } else if( !strncmp( line, "UTILITY_TYPE", strlen( "UTILITY_TYPE" ) ) ) {
      char tmp[ PATH_LENGTH ];
      if( get_next_token( tmp, &line[ strlen( "UTILITY_TYPE" ) ] ) ) { return 1; }
      utility_type = atoi( tmp );
    } else if( !strncmp( line, "ALPHA", strlen( "ALPHA" ) ) ) {
      char tmp[ PATH_LENGTH ];
      if( get_next_token( tmp, &line[ strlen( "ALPHA" ) ] ) ) { return 1; }
      alpha = atof( tmp );
    } else if( !strncmp( line, "BETA", strlen( "BETA" ) ) ) {
      char tmp[ PATH_LENGTH ];
      if( get_next_token( tmp, &line[ strlen( "BETA" ) ] ) ) { return 1; }
      beta = atof( tmp );
    } else if( !strncmp( line, "LAMBDA", strlen( "LAMBDA" ) ) ) {
      char tmp[ PATH_LENGTH ];
      if( get_next_token( tmp, &line[ strlen( "LAMBDA" ) ] ) ) { return 1; }
      lambda = atof( tmp );
